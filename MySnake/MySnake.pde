...
int foodX = ((int)random(25)*20);
int foodY = ((int)random(25)*20);
...
void drawSnake() {
  rect(head.getX(), head.getY(), 20, 20); 
...
void drawTail() {
  ...
  for(Segment s :tailArray){
  rect(s.getX(), s.getY(),20,20);
  }
}
...
for(Segment v : tailArray){
if(head.getX()== v.getX() && head.getY()== v.getY() ){
